...
- TCL 电视, 首先确保你的电视已开启“远程调试”或“ADB 调试”功能（通常在设置中的开发者选项里）。目前该功能**仅在 TCL X11G 型号上进行了测试**，其他型号由于系统差异可能支持不佳。本程序使用纯 Python 实现，无需在系统中安装 `adb` 命令行工具。播放结束时，默认会自动返回 Emby App。
    * tcv x11g 打开adb调试方法：
